...
import { cadenceLabel, modeLabel, sourceDetailLabel, sourceLabel, statusLabel } from '../lib/sourceLabels'
import type { AlertRule, DataSourceStatus, MetricCategory, MetricSeries, WatchRow } from '../types'
...
function isDisplayIndex(index: { id: string; metric_id?: string; change_pct: number; spark: number[] }, metric?: MetricSeries | null) {
...
export default function TerminalDashboard() {
  const { indices, watchlist, series, defaultMetricId, sources, alertRules, coverage, mode, lastUpdated, cacheState, loading, error } = useMarketData()
  ...
  const activeSourceCount = coverage?.live_sources ?? liveSources
  const totalSourceCount = coverage?.sources_total ?? sources.length
    ...
    >
      ...
      <div className="flex flex-1 flex-col overflow-visible xl:min-h-0">
        <section className="shrink-0 border-b border-slate-200 bg-white/85 px-4 py-2 backdrop-blur md:px-5">
          <div className="flex flex-col gap-2 xl:flex-row xl:items-center xl:justify-between">
            <div className="flex min-w-0 flex-wrap items-center gap-2">
              ...
              <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-semibold text-slate-600">
                准实时 {nearRealtimeSignals}
              </span>
              <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-semibold text-slate-600">
                {activeSourceCount}/{totalSourceCount} 源在线
              </span>
            </div>
            ...
            <div className="shrink-0 border-b border-slate-200 bg-gradient-to-r from-white via-sky-50/60 to-white px-4 py-3">
              <div className="flex flex-wrap items-start justify-between gap-4">
                ...
                <div className="flex shrink-0 items-end gap-5 border-l border-slate-200 pl-5">
                  ...
                  <div className={`pb-1 font-mono text-sm font-semibold ${(selectedMetric?.change_pct || 0) >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                    {(selectedMetric?.change_pct || 0) >= 0 ? '+' : ''}
                    {(selectedMetric?.change_pct || 0).toFixed(2)}%
                  </div>
                  {selectedWatchRow ? (
                    <button
                      type="button"
            ...
            <div className="grid flex-1 gap-3 overflow-visible p-3 xl:min-h-[34rem] xl:grid-rows-[minmax(30rem,1fr)_auto]">
              ...
              <div className="grid gap-3 border-t border-slate-100 pt-2 lg:grid-cols-[minmax(0,1fr)_20rem]">
                <div className="grid gap-3 sm:grid-cols-4">
                  ...
                  <MetaCell label="历史点数" value={`${selectedMetric?.points.length || 0} 个样本`} />
                </div>
          ...
          <aside className="grid gap-3 overflow-visible xl:min-h-0 xl:grid-rows-[minmax(30rem,1fr)_auto_auto]">
            ...
            <section className="rounded-xl border border-slate-200 bg-white p-3 shadow-[0_12px_28px_rgba(15,23,42,0.05)]">
              <div className="flex items-center justify-between gap-2 border-b border-slate-100 pb-2">
                ...
                <div className="text-[10px] text-slate-500">{loading ? '同步中' : `${liveSources} 实时 · ${degradedSources} 降级 · ${disabledSources} 待配`}</div>
              </div>
              <div className="mt-2 divide-y divide-slate-100">
                <StatusLine
                  label="本地事件图谱"
                  value={`${(coverage?.ground_news_records ?? groundNewsSource?.records ?? 0).toLocaleString('zh-CN')} 记录`}
        ...
        <footer className="flex shrink-0 items-center justify-between gap-3 border-t border-slate-200 bg-white px-4 py-2 text-[11px] text-slate-500 md:px-5">
          <span>{cacheState === 'hit' ? '已命中后端缓存' : '已刷新后端聚合结果'}</span>
          <span>{lastUpdated ? `更新于 ${lastUpdated.slice(0, 19).replace('T', ' ')} UTC` : '等待时间戳'}</span>
        </footer>
      </div>
    </div>
  )
...
function IndexTape({
  ...
  indices: { id: string; name: string; value: number; change_pct: number; metric_id?: string }[]
